...
function App() {
  const {
    user, products, cart, orders, addressBook, viewHistory, dynamicChats,
    toast, showToast, toggleLike, addComment, sellerReply, toggleCommentLike,
    updateProductStatus, deleteProduct, publishProduct, incrementViews, markAsSold,
    addToCart, updateQty, removeFromCart, clearCart, cartCount,
    checkout, updateOrderStatus, handleUserLogin, handleLogout, handleUpdateUser,
    setAddressBook, setViewHistory, setDynamicChats,
  } = useApp();
  ...
  }, [incrementViews, setViewHistory, navigate]);
  ...
  }, [dynamicChats, user, setDynamicChats, navigate]);
  ...
  // 从路由参数获取商品 ID
  const productIdFromRoute = location.pathname.match(/^\/product\/(.+)$/)?.[1];
  const selectedProduct = productIdFromRoute ? products.find(p => p.id === productIdFromRoute) : null;

  // 从路由参数获取聊天 ID
  const chatIdFromRoute = location.pathname.match(/^\/chat\/(.+)$/)?.[1];
  const selectedChat = chatIdFromRoute
    ? dynamicChats.find(c => c.id === chatIdFromRoute) || messagesData.chats.find(c => c.id === chatIdFromRoute)
    : null;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-eco-50/60 to-white">
      <Header
        ...
        onOpenViewHistory={() => navigate('/history')}
        cartCount={cartCount}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
        ...
        {/* 数据看板 */}
        {location.pathname === '/dashboard' && <Dashboard onClose={() => navigate('/')} />}
      ...
      {user && (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-35 bg-white/95 backdrop-blur-lg border-t border-slate-200 px-2 py-2 shadow-2xl">
          <div className="flex items-center justify-around">
            <button onClick={() => navigate('/dashboard')} className="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl hover:bg-eco-50 text-slate-600 hover:text-eco-700 transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
              <span className="text-[10px] font-semibold">数据</span>
            </button>
            <button onClick={() => navigate('/my-listings')} className="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl hover:bg-eco-50 text-slate-600 hover:text-eco-700 transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
              <span className="text-[10px] font-semibold">发布</span>
            </button>
            <button onClick={() => navigate('/my-favorites')} className="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl hover:bg-eco-50 text-slate-600 hover:text-eco-700 transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
              <span className="text-[10px] font-semibold">收藏</span>
            </button>
            <button onClick={() => navigate('/cart')} className="relative flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl hover:bg-eco-50 text-slate-600 hover:text-eco-700 transition-colors">
            ...
            <button onClick={() => navigate('/profile')} className="flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl hover:bg-eco-50 text-slate-600 hover:text-eco-700 transition-colors">
